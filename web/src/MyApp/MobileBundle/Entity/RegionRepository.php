...
class RegionRepository extends EntityRepository
{
	...
	public function getAdminRegions($page, $limit)
	{
		...
		$sql = 'SELECT p.id, p.nom_fr, p.nom_en FROM MyAppMobileBundle:Regions p ORDER BY p.nom_fr ASC';
		$query = $this->getEntityManager()->createQuery($sql);
	...
	{
		$sql = 'SELECT p FROM MyAppMobileBundle:Regions p WHERE p.id =:id';
		$query = $this->getEntityManager()->createQuery($sql);
		$query->setParameter('id', $id);
		return $query->getResult();
	}
	...
	{
		$sql = 'SELECT p.id, p.nom_fr, p.nom_en FROM MyAppMobileBundle:Regions p WHERE p.nom_fr LIKE :letter';
		$query = $this->getEntityManager()->createQuery($sql);
	...
	{
		$sql = 'SELECT COUNT(p.id) AS nbregions FROM MyAppMobileBundle:Regions p';
		$query = $this->getEntityManager()->createQuery($sql);
		return $query->getResult();
	}
	...
	{
		$sql = 'SELECT p.id, p.nom_fr FROM MyAppMobileBundle:Regions p';
		$query = $this->getEntityManager()->createQuery($sql);
		return $query->getResult();
	}
	...
	{
		$sql = 'SELECT p FROM MyAppMobileBundle:Regions p WHERE p.nom_fr =:word OR p.nom_en =:word GROUP BY p.province_etat_id';
		$query = $this->getEntityManager()->createQuery($sql);
		$query->setParameter('word', $word);
		return $query->getResult();
	}
	...
	{
		$sql = 'SELECT p FROM MyAppMobileBundle:Regions p WHERE p.id IN (:tab)';
		$query = $this->getEntityManager()->createQuery($sql);
	...
	{
		$sql = 'SELECT p FROM MyAppMobileBundle:Regions p WHERE p.nom_fr =:word OR p.nom_en =:word OR p.repertoire_fr =:word OR p.repertoire_en =:word';
		$query = $this->getEntityManager()->createQuery($sql);
		$query->setParameter('word', $word);
		return $query->getResult();
	}
	 ...
	 * Affiche toutes les régions de la province ordonnées par nom_fr ASC
	 */
	public function getRegionsDeLaProvince()
	{
	...
	{         
		$sql = 'SELECT p FROM MyAppMobileBundle:Regions p WHERE p.id IN (:id)';
		$query = $this->getEntityManager()->createQuery($sql);
		$query->setParameter('id', $id);
		return $query->getResult();
	}
	...
	{
		if($lang == "fr"){
			$sql = 'SELECT p.nom_fr FROM MyAppMobileBundle:Regions p';
			$query = $this->getEntityManager()->createQuery($sql);
		}else{
			$sql = 'SELECT p.nom_en FROM MyAppMobileBundle:Regions p';
			$query = $this->getEntityManager()->createQuery($sql);
		}
		return $query->getResult();
	}
	...
	{
		$sql = 'SELECT p FROM MyAppMobileBundle:Regions p WHERE p.province_etat_id = :id AND p.reservit_id <> 0 AND p.affiche = 1';
		$query = $this->getEntityManager()->createQuery($sql);
		$query->setParameter('id', $id);
		return $query->getResult();
	}
	...
	{
		$sql = 'SELECT p FROM MyAppMobileBundle:Regions p WHERE p.reservit_id = :id';
		$query = $this->getEntityManager()->createQuery($sql);
		$query->setParameter('id', $id);
		return $query->getResult();
	}
