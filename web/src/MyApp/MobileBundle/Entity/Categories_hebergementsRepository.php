...
class Categories_hebergementsRepository extends EntityRepository
{
	...
	{
		$sql = 'SELECT p.id, p.nom_fr, p.nom_en, p.image FROM MyAppMobileBundle:Categories_hebergements p';
	...
	{
		$sql = 'SELECT q FROM MyAppMobileBundle:Categories_hebergements q WHERE q.id <> 1';
		$query = $this->getEntityManager()->createQuery($sql);
		return $query->getResult();
	}
	...
	{
		$sql = 'SELECT q FROM MyAppMobileBundle:Categories_hebergements q WHERE q.nom_fr = :name';
		$query = $this->getEntityManager()->createQuery($sql);
	...
	public function getRechercheIdCategorieHebergement($name)
	{
		$sql = 'SELECT q.id, q.nom_fr, q.nom_en, q.repertoire_fr, q.repertoire_en, q.texte_fr, q.texte_en, q.page_titre_fr, q.page_titre_en, q.page_metadescription_fr, q.page_metadescription_en '.
                       'FROM MyAppMobileBundle:Categories_hebergements q WHERE q.nom_fr = :name OR q.nom_en = :name OR q.repertoire_fr =:name OR q.repertoire_en =:name';
		$query = $this->getEntityManager()->createQuery($sql);
		$query->setParameter('name', $name);
		return $query->getResult();
	}
	...
	{
		$sql = 'SELECT p, q FROM MyAppMobileBundle:Hebergements p JOIN p.categorie_hebergement_id q WHERE p.province_id = :province AND p.aprouver = 1 AND q.id = :tab';
		$query = $this->getEntityManager()->createQuery($sql);
        ...
        {
                $sql = 'SELECT q.repertoire_fr, q.repertoire_en FROM MyAppMobileBundle:Categories_hebergements q WHERE q.id = :id';
		$query = $this->getEntityManager()->createQuery($sql);
