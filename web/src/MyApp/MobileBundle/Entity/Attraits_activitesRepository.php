...
class Attraits_activitesRepository extends EntityRepository{
	...
	{
		$sql = 'SELECT p.id, p.nom_fr, p.nom_en, p.image FROM MyAppMobileBundle:Attraits_activites p';
	...
	{
		$sql = 'SELECT COUNT(p.id) AS nbActivite FROM MyAppMobileBundle:Attraits_activites p';
		$query = $this->getEntityManager()->createQuery($sql);
		return $query->getResult();
	}
	...
	{
		$sql = 'SELECT p.id, p.nom_fr, p.nom_en, p.image FROM MyAppMobileBundle:Attraits_activites p WHERE p.nom_fr = :word';
		$query = $this->getEntityManager()->createQuery($sql);
	...
	{	
		...
		$sql = 'SELECT p.id, p.nom_fr, p.nom_en, p.image FROM MyAppMobileBundle:Attraits_activites p ORDER BY p.nom_fr ASC';
		$query = $this->getEntityManager()->createQuery($sql);
	...
	{
		$sql = 'SELECT p.id, p.nom_fr, p.nom_en, p.image FROM MyAppMobileBundle:Attraits_activites p WHERE p.nom_fr LIKE :letter';
		$query = $this->getEntityManager()->createQuery($sql);
	...
	{
		$sql = 'SELECT p FROM MyAppMobileBundle:Attraits_activites p WHERE p.nom_fr = :word OR p.nom_en = :word';
		$query = $this->getEntityManager()->createQuery($sql);
		$query->setParameter('word', $searchword);
		return $query->getResult();
	}
	...
	{
		$sql = 'SELECT p FROM MyAppMobileBundle:Attraits_activites p WHERE p.id <> 135 ORDER BY p.nom_fr ASC';
		$query = $this->getEntityManager()->createQuery($sql);
		return $query->getResult();
	}
	...
	{
		$sql = 'SELECT p, q FROM MyAppMobileBundle:Attraits p JOIN p.attrait_activite_id q WHERE p.id IN (:tab) AND p.categorie_attrait = :categorie AND p.approuve = 1';
		$query = $this->getEntityManager()->createQuery($sql);
