...
class Attraits_serviceRepository extends EntityRepository{
	...
	{
		$sql = 'SELECT p.id, p.nom_fr, p.nom_en, p.image FROM MyAppGlobalBundle:Attraits_services p ORDER BY p.nom_fr';
	...
	{
		$sql = 'SELECT COUNT(p.id) AS nbService FROM MyAppGlobalBundle:Attraits_services p';
		$query = $this->getEntityManager()->createQuery($sql);
		return $query->getResult();
	}
	...
	{
		$sql = 'SELECT p.id, p.nom_fr, p.nom_en, p.image FROM MyAppGlobalBundle:Attraits_services p WHERE p.nom_fr = :word';
		$query = $this->getEntityManager()->createQuery($sql);
	...
	public function getAdminService($page, $limit)
	{
		...
		$sql = 'SELECT p.id, p.nom_fr, p.nom_en, p.image FROM MyAppGlobalBundle:Attraits_services p ORDER BY p.nom_fr ASC';
		$query = $this->getEntityManager()->createQuery($sql);
	...
	{
		$sql = 'SELECT p.id, p.nom_fr, p.nom_en, p.image FROM MyAppGlobalBundle:Attraits_services p WHERE p.nom_fr LIKE :letter';
		$query = $this->getEntityManager()->createQuery($sql);
