...
class DistancesRepository extends EntityRepository
{
	...
	{
            $sql = 'SELECT p FROM MyAppGlobalBundle:Distances p WHERE p.hebergement_id = :id ORDER BY p.nom_fr ASC ';
	...
	{
            //$sql = 'SELECT p FROM MyAppGlobalBundle:Distances p WHERE p.hebergement = :id AND p.distance <> 0 ';
            $sql = 'SELECT p FROM MyAppGlobalBundle:Distances p WHERE p.hebergement_id = :id ORDER BY p.activite_id ASC ';
            $query = $this->getEntityManager()->createQuery($sql);
            $query->setParameter('id', $id);
            return $query->getResult();
	}
	...
	{
            $valeurExclue = "";
            $sql = 'SELECT p FROM MyAppGlobalBundle:Distances p WHERE p.hebergement_id = :id AND p.distance <> :value ORDER BY p.distance ASC ';             
            $query = $this->getEntityManager()->createQuery($sql);
            $query->setParameter('id', $id);
        ...
        {
            $sql = 'SELECT p FROM MyAppGlobalBundle:Distances p WHERE p.hebergement_id = :idhebergement AND p.activite_id =:idactivite ';
            $query = $this->getEntityManager()->createQuery($sql);
	 ...
	 /**
        ...
        public function getRechercheLesDistancesDuClient($idactivite, $idhebergement)
        {
            ...
            $sql = 'SELECT p FROM MyAppGlobalBundle:Distances p WHERE p.hebergement_id = :idhebergement AND p.distance IN (:distance)';
            $query = $this->getEntityManager()->createQuery($sql);
            $query->setParameter('idhebergement', $idhebergement);
