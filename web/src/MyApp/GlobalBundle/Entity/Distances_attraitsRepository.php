...
class Distances_attraitsRepository extends EntityRepository
{
        ...
        {
            $sql = 'SELECT p FROM MyAppGlobalBundle:Distances_attraits p WHERE p.attrait_id = :idattrait AND p.activite_id =:idactivite ';
	...
	{
		$sql = 'SELECT p FROM MyAppGlobalBundle:Distances_attraits p WHERE p.attrait_id = :id ORDER BY p.activite_id ASC ';
		$query = $this->getEntityManager()->createQuery($sql);
