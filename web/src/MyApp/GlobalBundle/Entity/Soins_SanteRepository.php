...
class Soins_SanteRepository extends EntityRepository{
	...
	{
		$sql = 'SELECT COUNT(p.id) AS nbSoin FROM MyAppGlobalBundle:Soins_sante p';
	...
	{
		$sql = 'SELECT p.id, p.nom_fr FROM MyAppGlobalBundle:Soins_sante p';
		$query = $this->getEntityManager()->createQuery($sql);
		return $query->getResult();
	}
	...
	{
		$sql = 'SELECT p.id, p.nom_fr, p.nom_en FROM MyAppGlobalBundle:Soins_sante p WHERE p.nom_fr = :word';
		$query = $this->getEntityManager()->createQuery($sql);
	...
	public function getAdminSoin($page, $limit)
	{
		...
		$sql = 'SELECT p.id, p.nom_fr, p.nom_en FROM MyAppGlobalBundle:Soins_sante p ORDER BY p.nom_fr ASC';
		$query = $this->getEntityManager()->createQuery($sql);
	...
	{
		$sql = 'SELECT p.id, p.nom_fr, p.nom_en FROM MyAppGlobalBundle:Soins_sante p WHERE p.nom_fr LIKE :letter';
		$query = $this->getEntityManager()->createQuery($sql);
	...
	{
		$sql = 'SELECT p FROM MyAppGlobalBundle:Soins_sante p WHERE p.types_soins_sante_id is not null GROUP BY p.types_soins_sante_id';
		$query = $this->getEntityManager()->createQuery($sql);
		return $query->getResult();
	}
	...
	{
		$sql = 'SELECT p, q FROM MyAppGlobalBundle:Attraits p JOIN p.type_soins_id q GROUP BY q.id';
		$query = $this->getEntityManager()->createQuery($sql);
		return $query->getResult();
	}
	...
	{
		$sql = 'SELECT p, q FROM MyAppGlobalBundle:Attraits p JOIN p.type_soins_id q WHERE p.id IN (:client) AND p.approuve = 1 AND p.actif = 1 GROUP BY q.id';
		$query = $this->getEntityManager()->createQuery($sql);
	...
	{
		$sql = 'SELECT p FROM MyAppGlobalBundle:Soins_sante p ';
		$query = $this->getEntityManager()->createQuery($sql);
		return $query->getResult();
	}
	...
	public function getListTypeSoinPourMinisite($tabIdSoin)
	{
		...
		{
			$sql = 'SELECT p  FROM MyAppGlobalBundle:Soins_sante p WHERE p.id IN (:tabidsoin) ';
			$query = $this->getEntityManager()->createQuery($sql);
