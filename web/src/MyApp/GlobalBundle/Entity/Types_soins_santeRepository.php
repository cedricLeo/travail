...
class Types_soins_santeRepository extends EntityRepository{
	...
	{
		$sql = 'SELECT p.id, p.nom_fr, p.nom_en FROM MyAppGlobalBundle:Types_soins_sante p';
	...
	{
		$sql = 'SELECT p, q FROM MyAppGlobalBundle:Attraits p JOIN p.type_soins_id q WHERE p.region_id = :region AND q.id <> 126 AND p.approuve = 1 AND p.actif = 1 GROUP BY q.id';
		$query = $this->getEntityManager()->createQuery($sql);
	...
	{
		$sql = 'SELECT p, q FROM MyAppGlobalBundle:Attraits p JOIN p.type_soins_id q WHERE p.province_id = :province AND q.id <> 126 AND p.approuve = 1 AND p.actif = 1 GROUP BY q.id';
		$query = $this->getEntityManager()->createQuery($sql);
	...
	{
		$sql = 'SELECT p, q FROM MyAppGlobalBundle:Attraits p JOIN p.type_soins_id q WHERE p.ville_id = :ville AND q.id <> 126 AND p.approuve = 1 AND p.actif = 1 GROUP BY q.id';
		$query = $this->getEntityManager()->createQuery($sql);
	...
	{
		$sql = 'SELECT p FROM MyAppGlobalBundle:Types_soins_sante p WHERE p.nom_fr = :nom or p.nom_en = :nom';
		$query = $this->getEntityManager()->createQuery($sql);
	...
	{
		$sql = 'SELECT p, q FROM MyAppGlobalBundle:Attraits p JOIN p.type_soins_id q WHERE q.id = :soin';
		$query = $this->getEntityManager()->createQuery($sql);
	...
	public function getReturnTableauTypeSoin($tab, $id)
	{
		...
		for($i = 0; $i < count($tab); $i++){
			$sql = 'SELECT p, q FROM MyAppGlobalBundle:Attraits p JOIN p.type_soins_id q WHERE p.id = :id AND q.id = :soin';
			$query = $this->getEntityManager()->createQuery($sql);
