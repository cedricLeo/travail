...
class Provinces_etatsRepository extends EntityRepository{
	...
	{
		$sql = 'SELECT p.id, p.nom_fr, p.nom_en, p.image FROM MyAppGlobalBundle:Provinces_etats p';
		$query = $this->getEntityManager()->createQuery($sql);
		return $query->getResult();
	}
        ...
        /**
	 * Récupère les infos des provinces pour la version mobile.
	 */
	public function getAdminProvincesMobile()
	{
	...
	{
		$sql = 'SELECT p FROM MyAppGlobalBundle:Provinces_etats p WHERE p.nom_fr = :nom OR p.nom_en = :nom OR p.repertoire_fr =:nom OR p.repertoire_en =:nom';
		$query = $this->getEntityManager()->createQuery($sql);
	...
	{
		$sql = 'SELECT p.reservit_id, p.nom_fr, p.nom_en FROM MyAppGlobalBundle:Provinces_etats p WHERE p.pays_id =:id AND p.reservit_id is not null AND p.reservit_id <> 0 AND p.affiche = 1';
		$query = $this->getEntityManager()->createQuery($sql);
		$query->setParameter('id', $id);
		return $query->getResult();
	}
	...
	{
		$sql = 'SELECT p FROM MyAppGlobalBundle:Provinces_etats p WHERE p.reservit_id = :id';
		$query = $this->getEntityManager()->createQuery($sql);
		$query->setParameter('id', $id);
		return $query->getResult();
	}
