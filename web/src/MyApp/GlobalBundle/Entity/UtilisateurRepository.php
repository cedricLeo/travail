...
class UtilisateurRepository extends EntityRepository
{
	...
	public function getListUserGlobal()
	{
		...
		$sql = 'SELECT p FROM MyAppGlobalBundle:Utilisateur p WHERE p.role = :role';
	...
	{
		$sql = 'SELECT p FROM MyAppGlobalBundle:Utilisateur p WHERE p.nom =:session';
		$query = $this->getEntityManager()->createQuery($sql);
	...
	{
		$sql = 'SELECT p.courriel FROM MyAppGlobalBundle:Inscription_newletter p WHERE p.courriel = :id';
		$query = $this->getEntityManager()->createQuery($sql);
	...
	{
		$sql = 'SELECT p.date_connexion FROM MyAppGlobalBundle:Logs p WHERE p.utilisateur_id = :id ORDER BY p.date_connexion DESC';
		$query = $this->getEntityManager()->createQuery($sql);
	...
	{
		$sql = 'SELECT p.id FROM MyAppGlobalBundle:Utilisateur p WHERE p.username = :id';
		$query = $this->getEntityManager()->createQuery($sql);
		$query->setParameter('id', $tab);
		return $query->getResult();
	}
	...
	{
		$sql = 'SELECT p FROM MyAppGlobalBundle:Utilisateur p WHERE p.id = :id';
		$query = $this->getEntityManager()->createQuery($sql);
		$query->setParameter('id', $id);
		return $query->getResult();
	}
	...
	{
		$role = "ROLE_ADMIN";
		$sql = 'SELECT count(p.id) AS nbuser FROM MyAppGlobalBundle:Utilisateur p WHERE p.role = :role';
		$query = $this->getEntityManager()->createQuery($sql);
	...
	public function getRetourneListeClient($page, $limit)
	{
		...
		$sql = 'SELECT p FROM MyAppGlobalBundle:Utilisateur p WHERE p.role =:role ORDER BY p.nom ASC';
		$query = $this->getEntityManager()->createQuery($sql);
	...
	{
		$sql = 'SELECT p, q FROM MyAppGlobalBundle:Utilisateur p JOIN p.hebergements q WHERE p.id =:id';
		$query = $this->getEntityManager()->createQuery($sql);
		$query->setParameter('id', $id);
		return $query->getResult();
	}
	...
	{
		$sql = 'SELECT p FROM MyAppGlobalBundle:Utilisateur p WHERE p.username =:search OR p.nom =:search';
		$query = $this->getEntityManager()->createQuery($sql);
	...
	{
		$sql = 'SELECT COUNT(p.id) AS nblettre FROM MyAppGlobalBundle:Utilisateur p WHERE p.nom LIKE :lettre';
		$query = $this->getEntityManager()->createQuery($sql);
	...
	{
		$paginator = ($page - 1) * $limit;
		$sql = 'SELECT p FROM MyAppGlobalBundle:Utilisateur p WHERE p.nom LIKE :lettre ORDER BY p.nom ASC';
		$query = $this->getEntityManager()->createQuery($sql);
		$query->setFirstResult($paginator);
		$query->setMaxResults($limit);
		$query->setParameter('lettre', $lettre.'%');
	...
	{
		$sql = 'SELECT p FROM MyAppGlobalBundle:Utilisateur p WHERE p.nom LIKE :lettre';
		$query = $this->getEntityManager()->createQuery($sql);
		$query->setParameter('lettre', $lettre.'%');
		return $query->getResult();
	}
	...
	{
		$sql = 'SELECT p.nom_fr FROM MyAppGlobalBundle:Hebergements p WHERE p.aprouver = 1 AND p.actif = 1';
		$query = $this->getEntityManager()->createQuery($sql);
		$tab = array($query->getResult());
		/*$sql2 = 'SELECT p.nom_fr FROM MyAppGlobalBundle:Attraits p WHERE p.approuve = 1 AND p.actif = 1';
