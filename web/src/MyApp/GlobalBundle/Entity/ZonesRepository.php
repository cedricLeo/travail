...
class ZonesRepository extends EntityRepository{
	...
	public function getZones($page, $limit)
	{
		...
		$sql = 'SELECT p.id, p.nom_fr, p.nom_en FROM MyAppGlobalBundle:Zones p ORDER BY p.nom_fr ASC';
	...
	{
		$sql = 'SELECT p.id, p.nom_fr, p.nom_en FROM MyAppGlobalBundle:Zones p WHERE p.nom_fr LIKE :letter';
		$query = $this->getEntityManager()->createQuery($sql);
	...
	{
		$sql = 'SELECT p.id, COUNT(p.id) AS nbzones FROM MyAppGlobalBundle:Zones p';
		$query = $this->getEntityManager()->createQuery($sql);
		return $query->getResult();
	}
	...
	{
		$sql = 'SELECT p FROM MyAppGlobalBundle:Zones p WHERE p.id IN (:zone1, :zone2, :zone3, :zone4)';
		$query = $this->getEntityManager()->createQuery($sql);
	...
	{
		$sql = 'SELECT p FROM MyAppGlobalBundle:Zones p WHERE p.region_id =:id ORDER BY p.nom_fr ASC';
		$query = $this->getEntityManager()->createQuery($sql);
		$query->setParameter('id', $id);
		return $query->getResult();
	}
	...
	{
		$sql = 'SELECT p.id, p.nom_fr, p.nom_en FROM MyAppGlobalBundle:Zones p WHERE p.nom_fr=:word OR p.nom_en=:word';
		$query = $this->getEntityManager()->createQuery($sql);
	...
	{
		$sql = 'SELECT p.id, p.nom_fr FROM MyAppGlobalBundle:Zones p';
		$query = $this->getEntityManager()->createQuery($sql);
		return $query->getResult();
	}
	...
	{	
		$sql = 'SELECT p.id, p.nom_fr FROM MyAppGlobalBundle:Zones p WHERE p.nom_fr =:nomFR';
		$query = $this->getEntityManager()->createQuery($sql);
